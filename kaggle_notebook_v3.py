#!/usr/bin/env python3
"""
DS-Tuned v3 — Kaggle Notebook (5 cells)

Cell 0: Install deps + restart kernel
Cell 1: Training (QLoRA on Qwen2.5-Coder-14B)
Cell 2: Upload adapter to Kaggle Models
Cell 3: Hard eval (keyword-based, FT vs Base)
Cell 4: Constraint eval (multi-constraint, FT vs Base)

Instructions:
1. Upload train.jsonl + valid.jsonl as a Kaggle dataset
2. Create new notebook, GPU T4 x2, Internet ON
3. Add your training data dataset
4. Copy each cell into separate notebook cells
5. Run Cell 0, then after kernel restart, Run All Below from Cell 1
"""

# =====================================================================
# CELL 0 — Install dependencies and restart kernel
# =====================================================================
# Run this cell first, then the kernel restarts automatically.
# After restart, skip this cell and run cells 1-4.

import subprocess
subprocess.run(
    ["pip", "install", "-q", "unsloth", "datasets", "trl", "kagglehub"],
    check=True,
)
import os
os.kill(os.getpid(), 9)
...
# ── Format and tokenize with assistant-only masking ──────────────
...
def tokenize_with_masking(examples):
    ...
    # Get special token IDs for ChatML masking
    ...
    for convos in examples[text_field]:
        ...
        # Mask everything to -100, then unmask assistant content
            ...
            if input_ids[i] == im_start_id:
                ...
                if candidate == assistant_tokens:
                    ...
                    if content_end < len(input_ids):
                        content_end += 1  # include im_end token
...
print("\nDone! Adapter saved to: /kaggle/working/lora_adapter/")
...
MODEL_HANDLE = "jacks_423/datasci-coder-14b-lora/pytorch/v1"

print(f"Uploading adapter from {ADAPTER_DIR} to Kaggle Models...")
print(f"Handle: {MODEL_HANDLE}")

# List files being uploaded
total_size = 0
for fname in os.listdir(ADAPTER_DIR):
    fsize = os.path.getsize(os.path.join(ADAPTER_DIR, fname))
    total_size += fsize
...
try:
    kagglehub.model_upload(
        handle=MODEL_HANDLE,
        local_model_dir=ADAPTER_DIR,
        license_name="apache-2.0",
    )
    print(f"\nAdapter uploaded successfully!")
    print(f"Access at: https://www.kaggle.com/models/jacksonsm/datasci-coder-14b-lora")
except Exception as e:
    print(f"\nUpload failed: {e}")
    print("The adapter is still saved at /kaggle/working/lora_adapter/")
    print("You can download it manually from the notebook output files.")


# =====================================================================
# CELL 3 — Hard Eval (FT vs Base, keyword-based)
# =====================================================================
...
for name in ["model", "trainer", "tokenizer", "tokenized", "dataset"]:
    if name in globals():
        del globals()[name]
gc.collect()
...
# Pass ADAPTER_DIR directly — Unsloth reads adapter_config.json to find the base
# model and loads it natively, avoiding the PeftModel wrapper + RoPE shape bug.
...
gc.collect()
torch.cuda.empty_cache()
print("\nModels freed — GPU ready for constraint eval cells.")


# =====================================================================
# CELL 4 — Constraint Eval (FT vs Base)
# =====================================================================
